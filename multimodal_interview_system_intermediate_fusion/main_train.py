...
def main():
    ...
    if args.stage is not None:
        ...
        # Avoid redundant nesting (e.g., checkpoints/stage1/stage1)
        # Check if the base path already ends with "stage1", "stage2", etc.
        stage_suffix = f"stage{args.stage}"
        if not base_checkpoint_dir.rstrip('/\\').endswith(stage_suffix):
            stage_checkpoint_dir = Path(base_checkpoint_dir) / stage_suffix
            checkpoint_cfg['save_dir'] = str(stage_checkpoint_dir)
            logger.info(f"Auto-appended stage suffix to save_dir: {checkpoint_cfg['save_dir']}")
    ...
    logger.info("=" * 80)
    logger.info("Multimodal Interview Analysis System - Training (Intermediate Fusion)")
    if args.stage:
    ...
    if args.resume:
        ...
        if reset_best_tracking:
            ...
            logger.info(
                "Reset best-model tracking for this stage "
