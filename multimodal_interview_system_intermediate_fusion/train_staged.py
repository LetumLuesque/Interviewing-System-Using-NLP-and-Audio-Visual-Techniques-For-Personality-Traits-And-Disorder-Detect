...
def find_latest_checkpoint(checkpoint_dir="checkpoints", stage_num=None):
...
Examples:
  ...
  python train_staged.py --resume_from_stage 1 --data_dir path/to/mdpe_dataset --seumld_dir path/to/seumld
    ...
    if args.resume_from_stage:
        latest_checkpoint = find_latest_checkpoint(stage_num=args.resume_from_stage)
    ...
    for stage_num in stages_to_run:
        ...
        if stage_num > 1 and not current_checkpoint:
            ...
            current_checkpoint = find_latest_checkpoint(stage_num=stage_num - 1)
        ...
        if not success:
            ...
            latest_checkpoint = find_latest_checkpoint(stage_num=stage_num)
        ...
        current_checkpoint = find_latest_checkpoint(stage_num=stage_num)
        if current_checkpoint:
