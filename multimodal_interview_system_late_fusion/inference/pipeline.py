...
class InferencePipeline:
    ...
    def process_interview(
        ...
        # (Note: process_video_file runs inference without culture, so we re-run it here if culture is present.
        #  Optimization: convert process_video_file to just return features, then run model once)
        if cultural_group_tensor is not None:
             # Extract features (we need to duplicate logic from process_video_file or just call model again)
             # Reuse predictions from process_video_file is HARD because it returns final output, not features.
             # So we re-run process_video_file logic but with cultural_group.
             pass 

        # Optimization: We already ran model in process_video_file WITHOUT culture. 
        # But we need to run it WITH culture.
        # Calling process_video_file was suboptimal if we have culture.
        # Let's direct call logic here.
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            audio_features = mfcc.unsqueeze(0).to(self.device)
             ...
             # Extract from video if no separate audio
             try:
        ...
        # Calculate ensemble confidence from individual modalities
        individual_preds = predictions.get('individual_predictions', {})
        conf_tensors = []
        for mod, preds in individual_preds.items():
            if preds is not None and 'confidence' in preds:
                conf_tensors.append(preds['confidence'])
                
        if conf_tensors:
            # Average the confidence across available modalities
            avg_conf_tensor = torch.stack(conf_tensors).mean(dim=0)
            confidence = avg_conf_tensor.cpu().numpy()[0]
        else:
            confidence = None
