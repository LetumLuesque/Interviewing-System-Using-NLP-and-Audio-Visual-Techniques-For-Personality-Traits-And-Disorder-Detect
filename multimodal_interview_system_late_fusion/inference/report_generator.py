...
class HRReportGenerator:
    ...
    def generate_report(
        ...
        Args:
            ...
            video_path: Path to video file (used to get duration)
    ...
    def _generate_timeline_script(self, duration=30):
        """Generate animated timeline chart using Chart.js with actual video duration"""
        # Use actual duration, fallback to 30 if 0
        num_points = max(duration, 30)
        return f'''
        ...
        (function() {{
            ...
            // Generate simulated data matching actual video duration
            const labels = Array.from({{length: numPoints}}, (_, i) => i + 's');
            const deceptionData = Array.from({{length: numPoints}}, () => 0.15 + Math.random() * 0.15);
            const voiceStress = Array.from({{length: numPoints}}, () => 0.25 + Math.random() * 0.2);
            const expressionConf = Array.from({{length: numPoints}}, () => 0.75 + Math.random() * 0.15);
    ...
    <body>
        ...
        {self._generate_timeline_script(duration)}
    </body>
    </html>"""
