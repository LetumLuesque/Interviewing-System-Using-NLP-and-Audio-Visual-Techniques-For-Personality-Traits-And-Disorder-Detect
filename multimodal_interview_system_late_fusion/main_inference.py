...
def main():
    ...
    parser.add_argument('--fusion_method', type=str, default=None,
                       help='Override fusion method (concat, mean, weighted)')
    ...
    if args.fusion_method:
        ...
        # Map app UI names to Late Fusion internal names:
        #   App "Concat" → Late Fusion "learned" (MLP over concatenated predictions)
        #   App "Mean"   → Late Fusion "average"
        #   App "Weighted" → Late Fusion "weighted"
        late_fusion_method_map = {
            'concat': 'learned',
            'mean': 'average',
            'weighted': 'weighted',
        }
        fm = late_fusion_method_map.get(fm, fm)
        logger.info(f"Overriding fusion method to: {fm}")
        if 'fusion' not in model_config:
            model_config['fusion'] = {}
        if 'late_fusion' not in model_config['fusion']:
            model_config['fusion']['late_fusion'] = {}
        model_config['fusion']['late_fusion']['method'] = fm
