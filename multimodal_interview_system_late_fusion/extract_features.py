...
def extract_mdpe_features(data_dir: str, output_dir: str, limit: int = None):
    """
    ...
    for idx, row in tqdm(metadata.iterrows(), total=len(metadata), desc="Extracting MDPE"):
        ...
        try:
            ...
            if video_path and os.path.exists(video_path):
                ...
                torch.save(video_features, os.path.join(video_out, f"{feature_id}.pt"))
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='mdpe',
                        choices=['mdpe', 'seumld', 'all'],
                        help='Which dataset to extract (mdpe, seumld, or all)')
    ...
    if args.dataset == 'mdpe' or args.dataset == 'all':
        extract_mdpe_features(args.data_dir, args.output_dir, args.limit)
