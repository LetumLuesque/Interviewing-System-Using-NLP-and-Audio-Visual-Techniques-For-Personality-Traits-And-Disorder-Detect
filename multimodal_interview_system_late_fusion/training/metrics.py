...
class MetricsCalculator:
    ...
    def __init__(self, deception_threshold: float = 0.5):
        self.deception_threshold = float(deception_threshold)
        self.reset()
    ...
    def update(
        ...
        if deception_pred.dim() > 1:
            # For binary logits [B, 2], apply configurable threshold on positive-class probability.
            if deception_pred.shape[1] == 2:
                deception_prob = torch.softmax(deception_pred, dim=1)[:, 1]
                deception_pred_np = (deception_prob >= self.deception_threshold).long().detach().cpu().numpy()
            else:
                deception_pred_np = deception_pred.argmax(dim=1).detach().cpu().numpy()
        else:
            # If values are logits, convert to probabilities first.
            if torch.any(deception_pred < 0) or torch.any(deception_pred > 1):
                deception_prob = torch.sigmoid(deception_pred)
            else:
                deception_prob = deception_pred
            deception_pred_np = (deception_prob >= self.deception_threshold).long().detach().cpu().numpy()
